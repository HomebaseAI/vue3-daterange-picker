<template>
  <div class="calendar-time">
    <select v-model="hour" class="hourselect form-control mr-1" :disabled="readonly">
      <option v-for="h in hours" :key="h" :value="h">{{ formatNumber(h) }}</option>
    </select>
    :<select v-model="minute" class="minuteselect form-control ml-1" :disabled="readonly">
      <option v-for="m in minutes" :key="m" :value="m">{{ formatNumber(m) }}</option>
    </select>
    <template v-if="secondPicker">
      :<select v-model="second" class="secondselect form-control ml-1" :disabled="readonly">
        <option v-for="s in 60" :key="s - 1" :value="s - 1">{{ formatNumber(s - 1) }}</option>
      </select>
    </template>
...
export default {
  ...
  emits: {
    update: (payload: TimePickerValue) => true
  }
}
</script>
